-- 제약조건
/* 데이터베이스에는 무결점의 데이터가 필수.(=무결성의 원치)(값이 없을 수는 있지만 결점이 있음안됨.ex)프라이머리키가 중복이라거나 그런거?)
...
not null : 반드시 값이 필수!
unique : 반드시 유일한 값
check : 설정된 조건에 맞는 값만 저장 가능(반드시 조건을 ()안에 넣게 되어 있다)
...
학과장명 varchar(20));
-- (학과번호 char(글자갯수) primary key설정하면 자동으로 not null로 설정된다.)

create table 학생
(학번 char(5) primary key,
이름 varchar(20) not null,
생일 date not null,
연락처 varchar(20) unique, -- 없을 수도 있지만 다 있다는 가정으로 하나뿐인 번호만 존재하므로 유니크설정
...
/* primary key: not null 와  unique 을 두개 다 가지고 있다. 기본키
char :고정형문자
varchar : 가변형문자
date : 날짜형
*/
...
영화번호 char(5) not null references 영화테이블(영화번호),
