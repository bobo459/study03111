...
select * from 사원;
select 성별, 직위 from 사원
where 성별 = "여" and 직위 = "사원";  -- and 대신 && 사용가능!

/*11. 제품의 단가가 8000원 이상인 제품들 중에서 제품명 순으로 정렬하여 조회하세요.*/
select *from 제품;
select 제품명,단가 from 제품
where 단가 >= 8000
order by 제품명 ASC; -- asc : 오름차순 식, desc의 반대문구
...
where 직위 = '사원'
order by 입사일 desc  -- 단점 : 같은 날 입사자가 2명이상이면 이 식은 정보의 오류이다. 이런 점은 주의할 필요가 있다.
limit 1;
